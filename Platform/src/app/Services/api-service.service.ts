...
export class ApiService {
  ...
  authenticateUser(request: UserRequest): Observable<UserResponse> {
    return this.http.post<UserResponse>(`${this.baseUrl}/authenticate`, JSON.stringify(request), {headers});
  }

  analyzeCode(request: CodeRequest): Observable<CodeResponse> {
    return this.http.post<CodeResponse>(`${this.baseUrl}/analyze_code`, JSON.stringify(request), {headers});
  }

  testCode(request: CodeTestRequest): Observable<CodeTestResponse> {
    return this.http.post<CodeTestResponse>(`${this.baseUrl}/test_code`, JSON.stringify(request), {headers});
  }
  ...
  getFlowchart(request: FlowchartRequest): Observable<FlowchartResponse> {
    return this.http.post<FlowchartResponse>(`${this.baseUrl2}/get_flowchart`, JSON.stringify(request), {headers});
