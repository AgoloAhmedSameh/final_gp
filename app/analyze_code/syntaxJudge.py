...
def check_js_syntax(js_code):
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            return not(bool(run_result.stderr))  # Return false if there's a syntax error, otherwise True
    except FileNotFoundError:
        print("Warning: Node.js not found. Skipping JavaScript syntax check.")
        return True  # Assume syntax is correct if we can't check
...
def check_java_syntax(java_code):
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            return not(bool(compile_result.returncode))  # Returns true if syntax is correct, false if error
    except FileNotFoundError:
        print("Warning: Java compiler (javac) not found. Skipping Java syntax check.")
        return True  # Assume syntax is correct if we can't check
...
def check_cpp_syntax(cpp_code):
    try:
        ...
        print("Warning: C++ compiler (g++) not found. Skipping C++ syntax check.")
        return True  # Assume syntax is correct if we can't check
